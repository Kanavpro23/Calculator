...
def create_buttons(master):#made buttuons. Functions 
    ...
    b_percent = add_button(master, "%")
    b_percent.grid(row=5,column=3,columnspan=1)
...
def  clickButton(master,value):
    ...
    if value == "c":
        print("Value entered c")
        master.equation.delete(0,END)#cleared the display
    elif value == "=":
        print("Value entered =")
        if '√'  in current_equation:# nested statements are if else inside an if else statement. 
            print("Value entered sqrt")
        ...
        print("value entered is percent")
        result= re.findall("\d+", current_equation)#\d+ is an escapse character which represents a digit. 
        num = int(result[0])
        num = num/100
        master.equation.delete(0,END)
        master.equation.insert(-1, num)
...
def fact(value):#function of factorial, I used while loop,
    #a loop is something which repeats until the rules of the loop goes false. There are 3 things in a loop, the initialisation, the condition and the decrement counter.  
    prod=1
    while(value>1):
        prod= value*prod
        value-=1
...
def sqrt(value): # re is a regular expression library. It takes pattern as an input and finds that pattern in the given string. 
